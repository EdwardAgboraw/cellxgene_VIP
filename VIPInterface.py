...
def CellPopView(data):
    ...
    adata = createData(data)
    #subset by cluster
    adata = adata[adata.obs[Cluster_Key].isin([Cluster])] # generic pattern = "Annotations_Key" is in "Annotations"
    
    #Split by Condition
    adata_1 = adata[adata.obs[Condition_Key].isin([Conditions[0]])] # Annotation_Key is in Annotations
    adata_2 = adata[adata.obs[Condition_Key].isin([Conditions[1]])] #replace
